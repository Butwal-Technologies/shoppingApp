...
const App = () => {
  ...
  const jsonValue= await AsyncStorage.getItem('shopping1');
  return jsonValue != null ? JSON.parse(jsonValue) : null
}
  
  const addItem = async(text) => {
    const data= {id: uuid(), text};

    if (!text) {
      Alert.alert(
        'No item entered',
        'Please enter an item when adding to your shopping1 list',
        [
          {
            text: 'Understood',
            style: 'cancel',
...
if(retriveItem()==null){
  console.log('khaali');
       await AsyncStorage.setItem('shopping1', JSON.stringify(data));
}else{
  console.log('vareko',retriveItem());
await AsyncStorage.mergeItem('shopping1', JSON.stringify(data));}
return retriveItem()});
    }
  };
